...
namespace OSCTools {
	public class OSCBundleIn : OSCObject {
		...
		public OSCBundleIn(byte[] data, IPEndPoint sender) {
			...
			int index = 0;
			if (GetString(data, ref index) is not string bundleString)
				return;
			...
			try {
				...
				while (index < data.Length) {
					if (GetInt(data, ref index) is not int len)
						return;
				...
				OSCLog.WriteLine($"Got exception while parsing bundle: {error.Message}\nStack:\n{error.StackTrace}");
				corrupt = true;
