...
namespace Network
{
    internal class Server
    {
        ...
        void ConnectionBombRequest(OSCMessageIn message, IPEndPoint remote)
        {
            ...
            if (enemyPlayer == null)
                OSCLog.WriteLine($"{participant} does not exist have PlayerData. HOW");
        ...
        void ConnectionMarkReadyRequest(OSCMessageIn message, IPEndPoint remote)
        {
            ...
            if (result == 1)
            {
                if (_userSessionKey.TryGetValue(player.Username, out var session) && session.TryGetEnemyParticipant(player, out var enemyParticipant))
                {
